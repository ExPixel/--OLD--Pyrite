...
pub unsafe fn imgui_render_draw_lists(state: &ImGuiSupport, draw_data: &mut imgui::ImDrawData) {
	use std::mem;
...
pub fn imgui_new_frame(state: &mut ImGuiSupport, window_size: (u32, u32), hidpi_factor: f32) {
...
pub unsafe fn imgui_create_device_objects(state: &mut ImGuiSupport) {
	use std::mem;
...
pub unsafe fn imgui_create_fonts_texture(state: &mut ImGuiSupport) {
	use std::ptr;
	use std::mem;
...
pub fn imgui_render(state: &ImGuiSupport) {
	imgui::render();
