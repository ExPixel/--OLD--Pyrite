...
pub fn draw_tiles_text_mode(bgcnt: u16, xoffset: u16, yoffset: u16, memory: &GbaMemory, line: u16, bg_line: &mut GbaBGLine) {
	...
	let y = ((line as u32) + yoffset as u32) & __sh_mod;
	let mut column = 0;

	while column < 240 {
		// #TODO handle mosaic
		let x = (column + (xoffset as u32)) & __sw_mod;
		let map_tile = x >> 3;
		let sc = (map_tile >> 5) + ((screen_height >> 8) << 1);
		let map_tile_data = 

		let _c = rand_color(sc);
		for i in 0..8 {
			bg_line[(column as usize) + i] = _c;
		}
