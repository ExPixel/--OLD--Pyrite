...
impl GbaLcd {
	...
	pub fn render_line(&mut self, memory: &mut GbaMemory, line: u16, line_buffer: &mut [u8]) {
		...
		match dispcnt & 0x7 {
			0 => mode0::render_mode_0(dispcnt, memory, line, line_buffer),
			1 => mode1::render_mode_1(dispcnt, memory, line, line_buffer),
			2 => mode2::render_mode_2(dispcnt, memory, line, line_buffer),
			3 => mode3::render_mode_3(dispcnt, memory, line, line_buffer),
			4 => mode4::render_mode_4(dispcnt, memory, line, line_buffer),
			5 => mode5::render_mode_5(dispcnt, memory, line, line_buffer),
			_ => unreachable!()
		}
	}
}
...
#[inline(always)]
pub fn convert_rgb5_to_rgb8(rgb5: u16) -> (u8, u8, u8){
	(
