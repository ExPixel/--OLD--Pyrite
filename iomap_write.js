...
function _map8() {
	data8.forEach(function(item) {
  	d8map[item.offset] = item.name + " = value";
  });
}

function _map16() {
	data16.forEach(function(item) {
  	d16map[item.offset] = item.name + " = value";
    d8map[item.offset] = shiftc(item.name, 0, 0xFF, 16);
    d8map[item.offset + 1] = shiftc(item.name, 8, 0xFF, 16);
  });
}

function _map32() {
	data32.forEach(function(item) {
  	d32map[item.offset] = item.name + " = value";
    d16map[item.offset] = shiftc(item.name, 0, 0xFFFF, 32);
    d16map[item.offset + 2] = shiftc(item.name, 16, 0xFFFF, 32);
    d8map[item.offset] = shiftc(item.name, 0, 0xFF, 32);
    d8map[item.offset + 1] = shiftc(item.name, 8, 0xFF, 32);
    d8map[item.offset + 2] = shiftc(item.name, 16, 0xFF, 32);
    d8map[item.offset + 3] = shiftc(item.name, 24, 0xFF, 32);
  });
}
...
function shiftc(a, shift, mask, type) {
  ...
  if(shift >  0) s = "((value as u" + type + ") << " + shift + ")";
  else s = "(value as u" + type + ")";


  var _m;

  if(type == 32) { _m = 0xffffffff; }
  else { _m = ((((1 << type) >> 0) - 1) >> 0); }

  m = "0x" + ((((~((mask << shift) >>> 0)) >>> 0) & _m) >>> 0).toString(16)
  t = "{" + a + " &= " + m + "; " + a + " |= " + s + ";}"
  return t;
}
